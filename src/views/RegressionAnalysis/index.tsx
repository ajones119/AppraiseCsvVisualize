...
const RegressionAnalysis = ({ data }: RegressionAnalysisProps) => {
  const groups = useMemo(() => {
    const groups = CHARTS_DEFAULT_CONFIG.map((config) => {
      ...
      const regressionEndElement =
        regression.values[regression.values.length - 1];
        ...
        chartData: [
          {
            ...
            data: regression.values.map((value) => ({
              x: value[0],
              y: value[1],
            })),
          },
          {
            ...
            data: [
              { x: regressionStart, y: regression.line(regressionStart) },
              { x: regressionEnd, y: regression.line(regressionEnd) },
            ],
          },
        ],
      ...
      {groups.map((group) => (
        <div className={css.regressionCard}>
          ...
          <div className={css.subData}>
            <div>
              R<sup>2</sup>: {group.regression.r2.toFixed(3)}
            </div>
            <div>
              Slope:{" "}
          ...
          {!group.hideValues && (
            <table className={css.table}>
              ...
              {Object.keys(group.occurranceMap).map((key, index) => (
                <tr
                  className={`${css.tableRow} ${index % 2 == 0 && css.secondary}`}
                >
                  <td>{key}</td>
                  ...
                  <td>
                    {new Intl.NumberFormat("en-US", {
                      style: "currency",
                      currency: "USD",
                    }).format(group.regression.line(Number(key)))}
                  </td>
                  <td>
                    {zScore(
                      Number(key),
                      group.regression.avg,
                      group.regression.stddev,
                    ).toFixed(2)}
                  </td>
                </tr>
              ))}
