...
export const getLinearRegression = (
  entries: MLSDataEntry[],
  xAxisKey: keyof MLSDataEntry = "closingDate",
  isXAxisDate = false,
): LinearRegressionData => {
  const filteredEntries = entries.filter(
    (entry) => entry.salePrice && entry[xAxisKey] !== undefined,
  );
  const values = filteredEntries
    .map((entry) => [
      Number(isXAxisDate ? entry[xAxisKey].getTime() : entry[xAxisKey] || 0),
...
export const filterDataByGroupFilter = (
  entries: MLSDataEntry[],
  filter: GroupFilter,
) => {
